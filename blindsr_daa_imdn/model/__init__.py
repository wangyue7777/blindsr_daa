...
class Model(nn.Module):
    def __init__(self, args, ckp, load=False, load_E=False):
        ...
        # if load:
        #     if load_E:
        #         model_param = torch.load(args.model_load)
        #         model_param_E = dict()
        #         for name, param in model_param.items():
        #             if 'E.' in name:
        #                 model_param_E[name[2:]] = param
        #         try:
        #             self.get_model().E.load_state_dict(
        #                 model_param_E,
        #                 strict=True
        #             )
        #         except RuntimeError:
    ...
    def load(self, apath, pre_train='.', resume=-1, cpu=False):
        ...
        if resume > 0:
            print('load')
            self.get_model().load_state_dict(
                torch.load(os.path.join(pre_train, 'model', 'model_{}.pt'.format(resume)), **kwargs),
                strict=True
            )
        ...
        if resume == -1:
            print('load for test')
            self.get_model().load_state_dict(
                torch.load(os.path.join(pre_train, 'model', 'model_best.pt'), **kwargs),
                strict=True
            )
