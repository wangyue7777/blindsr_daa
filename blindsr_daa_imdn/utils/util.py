...
def stable_anisotropic_gaussian_kernel(batch=1, kernel_size=21, theta=0, lambda_1=0.2, lambda_2=4.0):
    theta = torch.ones(batch).cuda() * theta * math.pi
    lambda_1 = torch.ones(batch).cuda() * lambda_1
    lambda_2 = torch.ones(batch).cuda() * lambda_2
...
def stable_isotropic_gaussian_kernel(batch=1, kernel_size=21, sig=4.0):
    x = torch.ones(batch).cuda() * sig
...
def stable_gaussian_kernel(batch, kernel_size=21, blur_type='iso_gaussian', sig=2.6, lambda_1=0.2, lambda_2=4.0, theta=0):
    if blur_type == 'iso_gaussian':
        return stable_isotropic_gaussian_kernel(batch=batch, kernel_size=kernel_size, sig=sig)
    elif blur_type == 'aniso_gaussian':
        return stable_anisotropic_gaussian_kernel(batch=batch, kernel_size=kernel_size, lambda_1=lambda_1, lambda_2=lambda_2, theta=theta)
...
class Gaussin_Kernel(object):
    ...
    def __call__(self, batch, random):
        ...
        # stable kernel
            ...
            return stable_gaussian_kernel(batch, kernel_size=self.kernel_size, blur_type=self.blur_type,
                                          sig=self.sig,
        ...
        with torch.no_grad():
            ...
            if label > 0:
                ...
                if torch.is_tensor(noise_level):
                    noise = torch.randn_like(lr_blured).view(-1, N, C, H_lr, W_lr).mul_(noise_level.view(-1, 1, 1, 1, 1)).view(-1, C, H_lr, W_lr)
                else:
